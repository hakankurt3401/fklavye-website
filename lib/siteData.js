...
export const saveNews = async (news) => {
  try {
    ...
    for (const item of news) {
      ...
      await supabase.from('news').insert([{ ...rest, sort_order: item_order || 0 }]);
    }
    return true;
  } catch (error) {
...
export const saveAnnouncements = async (announcements) => {
  try {
    ...
    for (const item of announcements) {
      const { id, item_order, ...rest } = item;
      await supabase.from('announcements').insert([{ ...rest, sort_order: item_order || 0 }]);
    }
    return true;
  } catch (error) {
...
export const saveGallery = async (gallery) => {
  try {
    ...
    for (const item of gallery) {
      ...
      await supabase.from('gallery').insert([{ ...rest, sort_order: order || 0 }]);
    }
    return true;
  } catch (error) {
...
export const saveVideos = async (videos) => {
  try {
    ...
    // Yeni videoları ekle - sadece tabloda olan sütunları gönder
    for (const item of videos) {
      const { id, order, ...rest } = item;
      // duration -> description dönüşümü
      const videoData = {
        ...rest,
