...
export default function Tarihcemiz() {
  ...
  const loadData = async () => {
    try {
      ...
      setContent(data['Tarihçemiz'] || null);
    } catch (error) {
  ...
  if (loading) {
    return (
      <Layout title="Tarihçemiz - F Klavye Uçan Parmaklar Derneği">
  ...
  return (
    <Layout title={`${content.title || 'Tarihçemiz'} - F Klavye Uçan Parmaklar Derneği`}>
      <div className="max-w-7xl mx-auto px-4 py-6">
        <div className="bg-white rounded-xl shadow p-6">
