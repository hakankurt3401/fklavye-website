...
export const saveNews = async (news) => {
  try {
    ...
    for (const item of news) {
      ...
      if (error) console.error('Haber ekleme hatası:', error);
    }
    return true;
  } catch (error) {
...
export const saveGallery = async (gallery) => {
  try {
    ...
    for (const item of gallery) {
      ...
      const { error } = await supabase.from('gallery').insert([albumData]);
      if (error) console.error('Galeri ekleme hatası:', error);
    }
    return true;
  } catch (error) {
...
export const saveVideos = async (videos) => {
  try {
    // Önce tüm videoları sil
    await supabase.from('videos').delete().neq('id', 0);
    
    // Sonra tüm videoları ekle
