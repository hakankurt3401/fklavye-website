...
export default function FKlavyeninHikayesi() {
  ...
  const loadData = async () => {
    try {
      ...
      setContent(data['F Klavyenin Hikayesi'] || null);
    } catch (error) {
  ...
  if (loading) {
    return (
      <Layout title="F Klavyenin Hikayesi - F Klavye Uçan Parmaklar Derneği">
  ...
  return (
    <Layout title={`${content.title || 'F Klavyenin Hikayesi'} - F Klavye Uçan Parmaklar Derneği`}>
      <div className="max-w-7xl mx-auto px-4 py-6">
        <div className="bg-white rounded-xl shadow p-6">
