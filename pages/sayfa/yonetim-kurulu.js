...
export default function YonetimKurulu() {
  ...
  const loadData = async () => {
    try {
      ...
      setContent(data['Yönetim Kurulu'] || null);
    } catch (error) {
  ...
  if (loading) {
    return (
      <Layout title="Yönetim Kurulu - F Klavye Uçan Parmaklar Derneği">
  ...
  return (
    <Layout title={`${content.title || 'Yönetim Kurulu'} - F Klavye Uçan Parmaklar Derneği`}>
      <div className="max-w-7xl mx-auto px-4 py-6">
        <div className="bg-white rounded-xl shadow p-6">
