...
export default function FKlavyeninMucidi() {
  ...
  const loadData = async () => {
    try {
      ...
      setContent(data['F Klavyenin Mucidi'] || null);
    } catch (error) {
  ...
  if (loading) {
    return (
      <Layout title="F Klavyenin Mucidi - F Klavye Uçan Parmaklar Derneği">
  ...
  return (
    <Layout title={`${content.title || 'F Klavyenin Mucidi'} - F Klavye Uçan Parmaklar Derneği`}>
      <div className="max-w-7xl mx-auto px-4 py-6">
        <div className="bg-white rounded-xl shadow p-6">
