...
export const saveVideos = async (videos) => {
  try {
    // Her video için tek tek işlem yap
    for (const item of videos) {
      ...
      // Eğer id varsa ve pozitifse güncelle, yoksa ekle
      if (id && id > 0) {
        await supabase.from('videos').update(videoData).eq('id', id);
      } else {
        const { data } = await supabase.from('videos').insert([videoData]).select().single();
        if (data) {
          console.log('Yeni video eklendi, DB ID:', data.id);
        }
      }
    }
    // Güncellenmiş listeyi döndür
    const { data: finalData } = await supabase.from('videos').select('*').order('sort_order', { ascending: true });
    return finalData || [];
  } catch (error) {
